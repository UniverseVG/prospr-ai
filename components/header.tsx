...
const Header = async () => {
  ...
  return (
    <header className="fixed top-0 w-full border-b border-border bg-background/80 backdrop-blur-md z-50 supports-[backdrop-filter]:bg-background/60">
      <nav className="mx-auto px-4 h-20 flex items-center justify-between">
        <Link href="/">
          <Image
            src="/logo.png"
            alt="prospr logo"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger>
                <Button className="flex items-center gap-1">
                  <StarIcon className="h-4 w-4 text-neutral" />
                  <span className="hidden md:block">Growth Tools</span>
                  <ChevronDownIcon className="h-4 w-4 text-neutral" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
                <DropdownMenuItem>
                ...
                <DropdownMenuItem>
                  ...
                  >
                    <PenBox className="h-4 w-4 text-secondary" />
